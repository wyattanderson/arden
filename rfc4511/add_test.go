...
func TestAddRequestRoundTripsAttributesAndCopiesValues(t *testing.T) {
	...
	photo, ok := decoded.Attributes.Lookup("jpegPhoto")
	require.True(t, ok)
	assert.Equal(t, AttributeValue{0x00, 0xff, 0x80}, photo.Values[0])
}

func TestAddRequestPreservesEmptyAttributeValues(t *testing.T) {
	request := &AddRequest{
		Attributes: NewAttributes([]Attribute{
	...
	var decoded AddRequest
	decode(t, request.BERPacket().Encode(), &decoded)
	assert.Equal(t, *request, decoded)
}

func TestAddRequestRejectsMalformedAttributeAtomically(t *testing.T) {
