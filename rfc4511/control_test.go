...
func TestControlOptionalFieldSemantics(t *testing.T) {
	tests := []struct {
		...
		{"defaults omitted", rfc4511.Control{Type: rfc4511.LDAPOID("1.2.3")}},
		{"critical", rfc4511.Control{Type: rfc4511.LDAPOID("1.2.3"), Criticality: true}},
		{"present empty value", rfc4511.Control{Type: rfc4511.LDAPOID("1.2.3"), HasValue: true, Value: []byte{}}},
		{"binary value", rfc4511.Control{Type: rfc4511.LDAPOID("1.2.3"), HasValue: true, Value: []byte{0x00, 0xff}}},
	}
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			encoded, err := test.in.AppendBER(nil)
			require.NoError(t, err)
			var got rfc4511.Control
			decode(t, encoded, &got)
			assert.Equal(t, test.in, got)
		})
	}

	encoded, err := (rfc4511.Control{Type: rfc4511.LDAPOID("1.2.3")}).AppendBER(nil)
	require.NoError(t, err)
	assert.Equal(t, []byte{0x30, 0x07, 0x04, 0x05, '1', '.', '2', '.', '3'}, encoded)
}

func TestControlValidationAndFieldOrdering(t *testing.T) {
	for _, control := range []rfc4511.Control{
		{},
		{Type: rfc4511.LDAPOID("1")},
		{Type: rfc4511.LDAPOID("1..2")},
	} {
		dst := []byte{0xde, 0xad}
	...
	for _, encoded := range malformed {
		prior := rfc4511.Control{Type: rfc4511.LDAPOID("9.9"), Criticality: true}
		requireDecodeError(t, encoded, &prior)
		assert.Equal(t, rfc4511.LDAPOID("9.9"), prior.Type)
	}
}
