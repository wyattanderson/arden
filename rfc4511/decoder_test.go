...
func TestSearchDecoderRejectsTruncationsAtomically(t *testing.T) {
	...
	entry := SearchResultEntry{
		...
		Attributes: NewAttributes([]Attribute{{Type: "cn", Values: []AttributeValue{[]byte("Smith"), {0, 0xff}}}}...),
	}
	encoded = entry.BERPacket().Encode()
	for length := range len(encoded) {
		prior := SearchResultEntry{ObjectName: "keep", Attributes: NewAttributes([]Attribute{{Type: "keep"}}...)}
		requireDecodeError(t, encoded[:length], &prior)
		assert.Equal(t, SearchResultEntry{ObjectName: "keep", Attributes: NewAttributes([]Attribute{{Type: "keep"}}...)}, prior)
	}
